...
const Safari = () => {
  ...
  const handleNavigate = (e) => {
    ...
    // Check if the input is a valid domain or URL
    const isUrl = /^https?:\/\//i.test(query) || (/^[a-zA-Z0-9-]+\.[a-zA-Z]{2,}(\/.*)?$/.test(query) && !query.includes(" "));

    if (isUrl) {
      let finalUrl = query;
      if (!/^https?:\/\//i.test(query)) {
        finalUrl = `https://${query}`; // Auto append protocol
      }
      setIframeSrc(finalUrl);
      setUrl(finalUrl);
      setInputValue(finalUrl);
    } else {
      // Real Google Search (igu=1 bypasses Google's iframe block for searches)
    ...
    <div className="flex flex-col h-full w-full bg-[#f1f1f1] font-sans text-gray-800 select-none">
      ...
      <div className="flex items-center justify-between px-4 py-2.5 bg-[#f6f6f6] border-b border-gray-300/80 drag-handle">
        ...
        <div className="flex items-center gap-4 w-1/4">
          <BsLayoutSidebar className="text-gray-500 text-lg hover:text-gray-800 cursor-pointer transition-colors" />
        ...
        <div className="flex-1 flex justify-center max-w-2xl relative">
          <form onSubmit={handleNavigate} className="flex items-center bg-white border border-gray-200/80 rounded-md px-3 py-1.5 shadow-sm focus-within:ring-[3px] focus-within:ring-blue-400/30 focus-within:border-blue-400/50 transition-all w-full max-w-xl relative overflow-hidden">

            {/* Safari style loading progress bar effect */}
            {isLoading && (
              <div className="absolute left-0 bottom-0 top-0 bg-blue-500/10 w-1/3 animate-pulse pointer-events-none" />
            )}
            ...
            <input
              ...
              className="flex-1 bg-transparent outline-none text-[13px] text-center focus:text-left text-gray-800 px-2 w-full placeholder-gray-400 font-medium transition-all z-10"
              placeholder="Search Google or enter website name"
            />

            <div className="z-10 flex items-center justify-center w-5">
              {iframeSrc && isLoading ? (
                <IoClose
      ...
      <div className="flex-1 overflow-hidden relative bg-white">
        ...
        {/* Loading Overlay */}
        {isLoading && iframeSrc && (
          <div className="absolute inset-0 bg-white/70 backdrop-blur-sm z-50 flex flex-col items-center justify-center transition-all duration-300">
            <div className="relative flex items-center justify-center">
              <div className="w-10 h-10 border-4 border-gray-200 border-t-blue-500 rounded-full animate-spin"></div>
            </div>
            <p className="mt-4 text-sm font-medium text-gray-600">Loading {inputValue}...</p>
            <button
              onClick={stopLoading}
              className="mt-5 px-5 py-1.5 bg-gray-100 hover:bg-gray-200 border border-gray-300 text-gray-700 rounded-full text-xs font-semibold transition-colors shadow-sm"
            >
              Cancel
            </button>
          </div>
        ...
        {iframeSrc ? (
          <iframe
            ...
            className={`w-full h-full border-none bg-white transition-opacity duration-300 ${isLoading ? 'opacity-50' : 'opacity-100'}`}
